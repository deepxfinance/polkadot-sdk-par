...
pub use crate::{
	...
	overlayed_changes::{
		...
		StorageTransactionCache, StorageValue, OverlayedEntry, MergeErr, MergeChange, Changes,
	},
...
mod execution {
		...
		B: Backend<H>,
	{
		...
		method: &'a str,
		call_data: &'a [u8],
		cache: &'a mut typed_cache::OverlayCache,
		...
		B: Backend<H>,
	{
		...
		fn execute_aux(&mut self, use_native: bool) -> (CallResult<Exec::Error>, bool) {
			...
			self.cache.enter_runtime();
			self.overlay
				.enter_runtime()
				.expect("StateMachine is never called from the runtime; qed");

			let mut ext = Ext::new(Some(self.cache), self.overlay, cache, self.backend, Some(&mut self.extensions));
...
mod tests {
	...
	fn append_storage_works() {
		...
		{
			let ext = Ext::new(None, &mut overlay, &mut cache, backend, None);
			assert_eq!(ext.storage(key.as_slice()), Some(vec![reference_data[0].clone()].encode()));
		}
	}
