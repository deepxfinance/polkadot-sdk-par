...
pub use crate::{
	...
	overlayed_changes::{
		...
		StorageTransactionCache, OverlayedEntry, MergeErr, MergeChange, Changes,
	},
...
mod execution {
	...
	where
		...
		H::Out: Ord + Codec,
	{
		Err(Box::new("`prove_range_read_with_child_with_size_on_trie_backend` not supported in `kvdb` mode".to_string()))
	}
		...
		H::Out: Ord + Codec,
	{
		...
		#[cfg(feature = "kvdb")]
		Err(Box::new("`read_child_proof_check_on_proving_backend` not supported in `kvdb` mode".to_string()))
	}
	...
	where
		...
		H::Out: Ord + Codec,
	{
		Err(Box::new("`read_range_proof_check_on_proving_backend` not supported in `kvdb` mode".to_string()))
	}
