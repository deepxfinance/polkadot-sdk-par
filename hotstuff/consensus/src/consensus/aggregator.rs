...
use crate::agg_signature::{AggregateSignature, BitVec};
...
impl<Engine: EngineBLS> QCMaker<Engine> {
	...
	pub fn append<B: Block>(
		...
		let author_i = match authorities.iter().enumerate()
			.find(|(_, (a, _))| *a == vote.voter)
			.map(|(i, _)| i)
		{
		...
		let signature = AggregateSignature::aggregate::<Engine>(
			self.mask.clone(),
			authorities,
			digest.as_ref(),
			self.votes.clone(),
