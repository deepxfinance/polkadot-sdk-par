...
impl<T: pallet::Config> sp_runtime::BoundToRuntimeAppPublic for Pallet<T> {
	type Public = T::AuthorityId;
}

impl<T: pallet::Config> OneSessionHandler<T::AccountId> for Pallet<T> {
...
impl<T: pallet::Config> FindAuthor<u32> for Pallet<T> {
...
impl<T: pallet::Config, Inner: FindAuthor<u32>> FindAuthor<T::AuthorityId>
...
impl<T: pallet::Config> IsMember<T::AuthorityId> for Pallet<T> {
	fn is_member(authority_id: &T::AuthorityId) -> bool {
		Self::authorities().iter().any(|id| id == authority_id)
	}
}

impl<T: pallet::Config> OnTimestampSet<T::Moment> for Pallet<T> {
